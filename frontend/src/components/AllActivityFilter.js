...
const AllActivityFilter = ({ filters, setFilters }) => {
  ...
  return (
    <div className="all-activity-filter-container">
      {["All", "Sale", "Listing", "Update", "Cancel", "Transfer", "Mint"].map((type) => (
        <div
          key={type}
          className={`all-activity-filter-button ${isActive(type) ? "active" : ""}`}
          onClick={() => toggleFilter(type)}
          style={(type === "Sale" ||  type === "Listing" || type === "Update" || type === "Cancel")? { paddingLeft: "4px" } : {}}
        >
...
export default AllActivityFilter;
