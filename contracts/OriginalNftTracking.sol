...
contract ONftTracking is ReentrancyGuard {
    ...
    uint256 public defaultDeadlineDays = 14;
    uint256 public defaulRejectiontDeadlineDays = 7;
    ...
    // Mapping from NFT contract + tokenId to all shipmentIds
    ...
    event DeliveryDeadlineUpdated(uint256 newDeadlineDays);
    event  RejectionDeadlineUpdated(uint256 newDeadlineDays);
...
function getAllShipmentsForNFT(address nft, uint256 tokenId) external view returns (uint256[] memory) {
    ...
    require(
        ...
        "Shipment closed or already delivered"
    );

    require(
        msg.sender == s.seller || s.isAuthorizedThirdParty[msg.sender],
        "Only seller or authorized third party"
    );

    // ✅ Require max entries BEFORE pushing data
    require(
        s.locations.length < maxEntries && s.timestamps.length < maxEntries,
        "Max entries reached"
    );
    ...
    function markAsDelivered(uint256 shipmentId) external {
        Shipment storage s = shipments[shipmentId];
        require(block.timestamp <= s.createdAt + defaultDeadlineDays * 1 days, "Deadline has passed");
    ...
    function rejectDelivery(uint256 shipmentId, RejectionReason reason) external onlyBuyer(shipmentId) nonReentrant {
        Shipment storage s = shipments[shipmentId];
        require(s.status == Status.Delivered, "Item not delivered yet");
        
         require(block.timestamp <= s.deliveredAt + defaulRejectiontDeadlineDays * 1 days, "Rejection window expired");
    ...
    function cancelShipmentBySeller(uint256 shipmentId) external onlySeller(shipmentId) nonReentrant {
        Shipment storage s = shipments[shipmentId];
        require(s.status != Status.Delivered && s.status != Status.Rejected && s.status != Status.Cancelled, "Cannot cancel now");
    ...
    function autoCancelIfLate(uint256 shipmentId) external {
        ...
        require(block.timestamp > s.createdAt + defaultDeadlineDays * 1 days, "Deadline not passed");
    ...
    // Ensure that the rejection window has passed
    require(block.timestamp > s.deliveredAt + defaulRejectiontDeadlineDays * 1 days, "Rejection window has not passed yet");

    uint256 sellerAmount = s.price; // The seller gets the full price
    ...
    function updateDefaultDeadline(uint256 days_) external onlyOwner {
        require(days_ >= 1 && days_ <= 30, "Keep between 1-30 days");
        defaultDeadlineDays = days_;
        emit DeliveryDeadlineUpdated(days_);
    }

    function updateDefaultRejectionDeadline(uint256 days_) external onlyOwner {
        require(days_ >= 1 && days_ <= 30, "Keep between 1-30 days");
        defaulRejectiontDeadlineDays = days_;
        emit RejectionDeadlineUpdated(days_);
    }
